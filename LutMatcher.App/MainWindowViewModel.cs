...
public sealed class MainWindowViewModel : INotifyPropertyChanged
{
    ...
    public List<int> CameraDevices { get; private set; } = [];
    public int SelectedCameraDevice { get; set; }
    ...
    public MainWindowViewModel()
    {
        ...
        RefreshCameraCommand = new RelayCommand(RefreshCameras);
        LoadReferenceCameraCommand = new RelayCommand(() => LoadCamera(true), () => CameraDevices.Count > 0);
        LoadTargetCameraCommand = new RelayCommand(() => LoadCamera(false), () => CameraDevices.Count > 0);
    ...
    private void LoadCamera(bool reference)
    {
        try
        {
            var mat = _loader.LoadFromCamera(SelectedCameraDevice);
            if (reference)
            {
                _reference?.Dispose();
                _reference = mat;
            ...
            AppendLog($"Loaded {(reference ? "Reference" : "Target")} from camera {SelectedCameraDevice}");
            NotifyButtons();
        }
        catch (Exception ex)
        {
    ...
    private void RefreshCameras()
    {
        try
        {
            CameraDevices = _loader.ProbeCameraDevices();
            if (CameraDevices.Count > 0 && !CameraDevices.Contains(SelectedCameraDevice))
            {
                SelectedCameraDevice = CameraDevices[0];
                OnPropertyChanged(nameof(SelectedCameraDevice));
            }

            OnPropertyChanged(nameof(CameraDevices));
            AppendLog(CameraDevices.Count == 0 ? "No camera/capture devices detected." : $"Detected devices: {string.Join(", ", CameraDevices)}");
